...
export default function InclusaoPoliticasPage() {
    ...
    const loadItems = () => {
        api.get('/cadastros/politicas')
            .then(res => setItems(res.data))
            .catch(err => {
                console.error(err);
                toast({ title: "Erro", description: "Erro ao carregar políticas.", variant: "destructive" });
            });
    };

    const handleAdd = () => {
        if (!newItemName.trim()) return;
        api.post('/cadastros/politicas', { nome: newItemName.toUpperCase() })
            .then(res => {
                setItems([...items, res.data]);
                setNewItemName("");
                toast({ title: "Sucesso", description: "Política cadastrada.", className: "bg-primary text-primary-foreground border-none" });
            })
            .catch(err => {
                console.error(err);
                toast({ title: "Erro", description: "Erro ao adicionar política.", variant: "destructive" });
            });
    };

    const handleConfirmEdit = () => {
        api.put(`/cadastros/politicas/${currentItem.id}`, { nome: editName.toUpperCase() })
            .then(res => {
                setItems(items.map(i => i.id === currentItem.id ? res.data : i));
                setIsEditOpen(false);
                toast({ title: "Atualizado", description: "Política alterada.", className: "bg-primary text-primary-foreground border-none" });
            })
            .catch(err => {
                console.error(err);
                toast({ title: "Erro", description: "Erro ao atualizar política.", variant: "destructive" });
            });
    };

    const handleConfirmDelete = () => {
        api.delete(`/cadastros/politicas/${currentItem.id}`)
            .then(() => {
                setItems(items.filter(i => i.id !== currentItem.id));
                setIsDeleteOpen(false);
                toast({ title: "Removido", description: "Política removida.", className: "bg-red-600 text-white border-none" });
            })
            .catch(err => {
                console.error(err);
                toast({ title: "Erro", description: "Erro ao remover política.", variant: "destructive" });
            });
    };
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-10">
            <div className="flex items-center gap-2 border-b pb-4">
                <h1 className="text-2xl font-bold tracking-tight text-primary">Inclusão de Políticas</h1>
            </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex justify-end">
                            <Button onClick={handleAdd} className="bg-primary hover:bg-primary/90 text-primary-foreground font-bold h-10 px-6">
                                <Plus className="mr-2 h-4 w-4" /> Incluir Política
                            </Button>
                        </div>
                    </div>
                </CardContent>
                ...
                <Table>
                    <TableHeader className="bg-white">
                        <TableRow>
                            ...
                            <TableHead className="font-bold">Política</TableHead>
            ...
            <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
                <DialogContent>
                    <DialogHeader><DialogTitle>Alterar Política</DialogTitle></DialogHeader>
