...
import { Search, Plus, AlertTriangle, Globe } from "lucide-react";
...
export default function InclusaoOrigemPage() {
    ...
    const handleAdd = () => {
        ...
        api.post('/cadastros/origens', { nome: newItemName.toUpperCase() })
            .then(res => {
                setItems([...items, res.data]);
                setNewItemName("");
                toast({ title: "Sucesso", description: "Origem adicionada.", className: "bg-primary text-primary-foreground border-none" });
            })
            .catch(err => {
                console.error(err);
                toast({ title: "Erro", description: "Erro ao adicionar origem.", variant: "destructive" });
            });
    };

    const handleConfirmEdit = () => {
        api.put(`/cadastros/origens/${currentItem.id}`, { nome: editName.toUpperCase() })
            .then(res => {
                setItems(items.map(i => i.id === currentItem.id ? res.data : i));
                setIsEditOpen(false);
                toast({ title: "Atualizado", description: "Origem alterada.", className: "bg-primary text-primary-foreground border-none" });
            })
            .catch(err => {
                console.error(err);
    ...
    const handleConfirmDelete = () => {
        api.delete(`/cadastros/origens/${currentItem.id}`)
            .then(() => {
                setItems(items.filter(i => i.id !== currentItem.id));
                setIsDeleteOpen(false);
                toast({ title: "Removido", description: "Origem removida.", className: "bg-red-600 text-white border-none" });
            })
            .catch(err => {
                console.error(err);
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-10">
            <div className="flex items-center gap-2 border-b pb-4">
                <h1 className="text-2xl font-bold tracking-tight text-primary">Origem de Venda</h1>
            </div>
                    ...
                    <div className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                placeholder="Digite a origem (Ex: Instagram)..."
                        ...
                        <div className="flex justify-end">
                            <Button onClick={handleAdd} className="bg-primary hover:bg-primary/90 text-primary-foreground font-bold h-10 px-6">
                                <Plus className="mr-2 h-4 w-4" /> Incluir Origem
                            </Button>
                        </div>
                    </div>
                </CardContent>
            ...
            <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
                <DialogContent>
                    <DialogHeader><DialogTitle>Alterar Origem</DialogTitle></DialogHeader>
