...
export default function DetalheSacolinhaPage() {
    ...
    const handleConfirmRestock = async () => {
        ...
        const handleForceAdd = async () => {
            ...
            setIsRestocking(true);
            try {
                // Force stock to 1 so it can be reserved
                await api.put(`/catalogo/pecas/${productToRestock.id}`, {
                    quantidade: 1
                });

                // The backend already resets status to DISPONIVEL when quantity > 0
                await handleAddItem(productToRestock.id);
                playBeep();
                ...
                <Dialog open={restockModalOpen} onOpenChange={setRestockModalOpen}>
                    <DialogContent>
                        ...
                        <DialogFooter className="flex flex-col sm:flex-row gap-2">
                            ...
                            <Button variant="secondary" onClick={handleForceAdd} disabled={isRestocking} className="sm:flex-1">
                                {isRestocking ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Plus className="mr-2 h-4 w-4" />}
                                Adicionar mesmo assim
                            </Button>
                            <Button onClick={handleConfirmRestock} disabled={isRestocking} className="bg-primary hover:bg-primary/90 sm:flex-1">
