...
export default function InclusaoLocaisPage() {
    ...
    const loadItems = () => {
        api.get('/cadastros/locais')
            .then(res => setItems(res.data))
            .catch(err => {
                console.error(err);
                toast({ title: "Erro", description: "Erro ao carregar locais.", variant: "destructive" });
            });
    };

    const handleAdd = () => {
        if (!newItemName.trim()) return;
        api.post('/cadastros/locais', { nome: newItemName.toUpperCase() })
            .then(res => {
                setItems([...items, res.data]);
                setNewItemName("");
                toast({ title: "Sucesso", description: "Local adicionado.", className: "bg-primary text-primary-foreground border-none" });
            })
            .catch(err => {
                console.error(err);
                toast({ title: "Erro", description: "Erro ao adicionar local.", variant: "destructive" });
            });
    };

    const handleConfirmEdit = () => {
        api.put(`/cadastros/locais/${currentItem.id}`, { nome: editName.toUpperCase() })
            .then(res => {
                setItems(items.map(i => i.id === currentItem.id ? res.data : i));
                setIsEditOpen(false);
                toast({ title: "Atualizado", description: "Local alterado.", className: "bg-primary text-primary-foreground border-none" });
            })
            .catch(err => {
                console.error(err);
                toast({ title: "Erro", description: "Erro ao atualizar local.", variant: "destructive" });
            });
    };

    const handleConfirmDelete = () => {
        api.delete(`/cadastros/locais/${currentItem.id}`)
            .then(() => {
                setItems(items.filter(i => i.id !== currentItem.id));
                setIsDeleteOpen(false);
                toast({ title: "Removido", description: "Local removido.", className: "bg-red-600 text-white border-none" });
            })
            .catch(err => {
                console.error(err);
                toast({ title: "Erro", description: "Erro ao remover local.", variant: "destructive" });
            });
    };
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-10">
            <div className="flex items-center gap-2 border-b pb-4">
                <h1 className="text-2xl font-bold tracking-tight text-primary">Locais de Estoque</h1>
            </div>
                    ...
                    <div className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                placeholder="Digite o local (Ex: Loja Centro)..."
                        ...
                        <div className="flex justify-end">
                            <Button onClick={handleAdd} className="bg-primary hover:bg-primary/90 text-primary-foreground font-bold h-10 px-6">
                                <Plus className="mr-2 h-4 w-4" /> Incluir Local
                            </Button>
                        </div>
                    </div>
                </CardContent>
            ...
            <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
                <DialogContent>
                    <DialogHeader><DialogTitle>Alterar Local</DialogTitle></DialogHeader>
