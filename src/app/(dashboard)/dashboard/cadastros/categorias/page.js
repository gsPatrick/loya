...
export default function InclusaoCategoriasPage() {
    ...
    const handleConfirmEdit = () => {
        api.put(`/cadastros/categorias/${currentItem.id}`, {
            ...
            .then(res => {
                ...
                setIsEditOpen(false);
                toast({ title: "Atualizado", description: "Categoria alterada." });
            })
            .catch(err => {
                console.error(err);
    ...
    const filteredItems = items.filter(i =>
        i.nome && i.nome.toLowerCase().includes(searchTerm.toLowerCase())
    );
        ...
        <div className="space-y-6 animate-in fade-in duration-500 pb-10">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex items-center gap-4">
                                {newItemFoto && (
                                    <img
                                        src={
                                            newItemFoto.startsWith("http")
                                                ? newItemFoto
                                                : `${api.defaults.baseURL.replace("/api/v1", "")}${newItemFoto}`
                                        }
                                        alt="Preview"
                                ...
                                <label className="cursor-pointer bg-gray-100 hover:bg-gray-200 text-gray-600 px-4 py-2 rounded text-sm font-medium transition-colors">
                                    ...
                                    <input type="file" className="hidden" accept="image/*"
                                        onChange={(e) => handleImageUpload(e, setNewItemFoto)} />
                                </label>
                            </div>
                        </div>
                            ...
                            <TableRow key={item.id} className="hover:bg-primary/5 border-b">
                                        ...
                                        <img
                                            src={
                                                item.foto.startsWith("http")
                                                    ? item.foto
                                                    : `${api.defaults.baseURL.replace("/api/v1", "")}${item.foto}`
                                            }
                                            alt=""
                                ...
                                <TableCell className="text-center">
                                    <Button size="sm"
                                        ...
                                        className="h-6 w-full bg-red-500 hover:bg-red-600 text-white text-[10px]"
                                    >APAGAR</Button>
                                </TableCell>
                            </TableRow>
                        ))}
            ...
            <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
                <DialogContent>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex items-center gap-4">
                                {editFoto && (
                                    <img
                                        src={
                                            editFoto.startsWith("http")
                                                ? editFoto
                                                : `${api.defaults.baseURL.replace("/api/v1", "")}${editFoto}`
                                        }
                                        alt="Preview"
                                        className="w-16 h-16 object-cover rounded border"
                                    />
                                ...
                                <label className="cursor-pointer bg-gray-100 hover:bg-gray-200 text-gray-600 px-4 py-2 rounded text-sm font-medium transition-colors">
                                    Alterar Imagem
                                    <input type="file" className="hidden" accept="image/*"
                                        onChange={(e) => handleImageUpload(e, setEditFoto)} />
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <DialogFooter>
                        <Button onClick={handleConfirmEdit} className="bg-primary text-primary-foreground">
                            Salvar
                        </Button>
                    </DialogFooter>
                </DialogContent>
